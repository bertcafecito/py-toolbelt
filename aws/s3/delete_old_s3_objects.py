...
def list_old_objects(s3_client, bucket: str, prefix: str, cutoff_date: datetime) -> List[Dict]:
    """
    ...
    print(f"Objects older than or equal to {cutoff_date.strftime('%Y-%m-%d')}: {len(objects_to_delete)}")
    
    return objects_to_delete


def delete_objects(s3_client, bucket: str, objects: List[Dict], batch_size: int = 1000, dry_run: bool = False):
    """
    ...
    if dry_run:
        ...
        for obj in objects[:10]:
            ...
            print(f"  - {obj['Key']} (Modified: {obj['LastModified']}, Size: {size_kb:.2f} KB)")
        if len(objects) > 10:
    ...
    for i in range(0, len(objects), batch_size):
        ...
        try:
            ...
            print(f"Deleted {deleted} objects (Progress: {deleted_count}/{len(objects)})")
...
def main():
    ...
    try:
        ...
        print(f"Objects with LastModified <= {cutoff_date.strftime('%Y-%m-%d')} will be deleted.\n")
