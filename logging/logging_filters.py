...
class SensitiveDataFilter(logging.Filter):
    ...
    PATTERNS = {
        ...
        # Private keys
        'private_key': re.compile(r'-----BEGIN (?:RSA |EC |OPENSSH |DSA |ENCRYPTED )?PRIVATE KEY-----[\s\S]*?-----END (?:RSA |EC |OPENSSH |DSA |ENCRYPTED )?PRIVATE KEY-----'),
        
        # JWT tokens
        'jwt': re.compile(r'\beyJ[A-Za-z0-9_\-]*\.eyJ[A-Za-z0-9_\-]*\.[A-Za-z0-9_\-]*\b'),
        
        # Database connection strings
        'db_connection': re.compile(r'\b(?:mysql|postgresql|postgres|mongodb|redis|mssql)://[^\s]+:[^\s]+@[^\s]+'),
    ...
    def filter(self, record):
        ...
        if record.args:
            if isinstance(record.args, dict):
                record.args = {k: self.mask_sensitive_data(str(v)) for k, v in record.args.items()}
